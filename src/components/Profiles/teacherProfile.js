...
function TeachrProfile() {
  const token = localStorage.getItem('token');
  const { teacher } = jwtDecode(token);
  console.log(teacher, "student123");
  ...
  useEffect(() => {
    if(teacher.length > 0){
      (async () => {
        try {
          const { data} = await axios.get(`/api/oneTeacher/${teacher}`, {
            headers: {
  ...
  const teacherId = teacher; // Ensure studentId is correctly obtained
  ...
  const handleUpload = (e) => {
    ...
    if (profile) {
      ...
      axios
        .put("/api/teacher/teacherId/profilePic", formData, {
          headers: {
            Authorization: localStorage.getItem("token"),
          },
        })
